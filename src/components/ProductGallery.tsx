...
const StarIcon = () => (
  <svg viewBox="0 0 24 24" className="pg-star-icon">
    <path
...
const CheckBadgeIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" className="pg-bullet-icon">
    <path
      fillRule="evenodd"
      clipRule="evenodd"
...
const bullets = [
  ...
  "Naturligt resultat som döljer tunnhårighet",
  "Sitter hela dagen – vind- och vattentålig",
];

function useViewerCount() {
  const [count, setCount] = useState(() => Math.floor(Math.random() * 50) + 40);
  useEffect(() => {
    const update = () => {
      setCount((c) => {
        const delta = Math.floor(Math.random() * 3) + 1;
        const next = c + (Math.random() > 0.5 ? delta : -delta);
        return Math.max(30, Math.min(110, next));
      });
    };
    const id = setInterval(update, Math.floor(Math.random() * 3000) + 5000);
    return () => clearInterval(id);
  }, []);
  return count;
}

const ProductGallery: React.FC<ProductGalleryProps> = ({ selectedPackage, onPackageChange }) => {
  ...
  return (
    <div className="pg-root">
      ...
      <div className="pg-gallery">
        <div className="pg-main-image">
          {images.map((img, i) => (
            <div key={img.id} className={`pg-image-slide ${i === activeIndex ? "active" : ""}`}>
      ...
      <div className="pg-info">
        ...
        <div className="pg-social-proof">
          ...
          <span className="pg-review-text">
            <strong>4,7/5</strong> · +3 475 recensioner
          </span>
        </div>

        {/* Title */}
        <h1 className="pg-title">Mello Hair Filler</h1>

        {/* Award badge */}
        <div className="pg-award">
          <div className="pg-award-badge">#1</div>
          <div className="pg-award-text">
            <div className="pg-award-title">BÄSTSÄLJARE 2024</div>
            <div className="pg-award-sub">Mest populära hårvård för herrar</div>
          </div>
        </div>
        ...
        {/* Viewer count */}
        <div className="pg-viewers">
          <span className="pg-viewer-dot" />
          <span className="pg-viewer-text">
            Säljer snabbt: <strong>{viewers} personer</strong> kollar just nu
          </span>
        </div>

        {/* Bundle selector + ATC */}
        <QuantityBreak selectedPackage={selectedPackage} onPackageChange={onPackageChange} />
      </div>
    </div>
  );
};
